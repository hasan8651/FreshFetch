...
const UserStats = () => {
  ...
  useEffect(() => {
    const fetchUserStats = async () => {
      ...
      try {
        ...
        setTotalOrderCount(res.data.totalOrder || res.data.orders?.length || 0);
      } catch (error) {
  ...
  const totalSpent = orders.reduce(
    (sum, order) => sum + (order.paymentStatus === "paid" ? (order.total || 0) : 0),
    0
  );
        ...
        <div className="mt-8 md:mt-0 relative z-10">
          <Link href="/profile">
            <motion.button 
              ...
              whileTap={{ scale: 0.95 }}
              className="bg-white text-gray-900 px-10 py-5 rounded-3xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-500 hover:text-white transition-colors shadow-2xl"
            >
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm">
          <h4 className="text-sm font-black text-gray-800 mb-8 uppercase tracking-widest flex items-center gap-2">
            <FiActivity className="text-indigo-500" /> Expense Analysis
          </h4>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={spendingData}>
                ...
                <Area type="monotone" dataKey="amount" stroke="#6366f1" strokeWidth={4} fill="url(#colorSpent)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="flex justify-between items-center mb-8">
            ...
            <Link href="/dashboard/my-orders" className="text-[10px] font-black text-gray-400 hover:text-indigo-600 transition-colors uppercase tracking-widest">
              View All
            </Link>
          </div>
          <div className="space-y-4">
            {orders.length > 0 ? (
              ...
              <div className="py-20 text-center text-gray-300 font-black text-[10px] uppercase tracking-[0.3em]">
                No orders yet
              </div>
            )}
          </div>
